...
const buildInitialHabitData = (morningHabits, eveningHabits) => {
  const allHabits = [...morningHabits, ...eveningHabits];
...
function AddBtn({ onClick }) {
  return (
    <button onClick={onClick} style={{ padding: "8px 14px", borderRadius: 8, border: "none", background: "#22c55e", color: "#000", fontSize: 16, fontWeight: 700, cursor: "pointer" }}>
      +
    </button>
  );
}
...
function DelBtn({ onClick }) {
  return (
    <button onClick={onClick} style={{ background: "none", border: "none", color: "#4b5563", cursor: "pointer", fontSize: 14, padding: "0 4px", lineHeight: 1 }}>
      ✕
    </button>
  );
}
...
function Card({ children, style }) {
  return (
    <div style={{ background: "#111", border: "1px solid #1f1f1f", borderRadius: 12, padding: 16, ...style }}>
      {children}
    </div>
  );
}
...
function ST({ t }) {
  return (
    <div style={{ fontSize: 12, color: "#86efac", fontFamily: "'Space Mono',monospace", marginBottom: 10, letterSpacing: 1 }}>
      {t}
    </div>
  );
}
...
function SaveBadge({ status }) {
  ...
  return (
    <div style={{
      ...
      color: s.color, fontFamily: "'Space Mono',monospace",
      fontSize: 11, padding: "6px 14px", borderRadius: 99,
      boxShadow: `0 0 12px ${s.color}44`, transition: "all 0.3s",
    }}>
      {s.label}
    </div>
  );
...
function LiveClock() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      <div style={{ fontSize: 11, color: "#86efac", fontFamily: "'Space Mono',monospace", letterSpacing: 2, opacity: 0.8 }}>
        1% Everyday. Focus.
      </div>
    ...
    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
      {Object.keys(TAG_COLORS).map((tag) => (
        <button key={tag} onClick={() => onChange(tag)} style={{
          ...
          border: `1px solid ${current === tag ? TAG_COLORS[tag].bg : "#333"}`,
          background: current === tag ? TAG_COLORS[tag].bg : "transparent",
          color: current === tag ? TAG_COLORS[tag].text : "#6b7280",
          fontSize: 10, cursor: "pointer", fontWeight: 600, transition: "all 0.15s",
        }}>{tag}</button>
      ))}
    </div>
  );
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ position: "relative", width: 110, height: 110 }}>
          <svg width="110" height="110" style={{ position: "absolute", top: 0, left: 0, transform: "rotate(-90deg)" }}>
            ...
            <circle cx="55" cy="55" r="48" fill="none" stroke="#22c55e" strokeWidth="4"
              strokeDasharray={`${2 * Math.PI * 48}`}
              strokeDashoffset={`${2 * Math.PI * 48 * (1 - progress)}`}
              strokeLinecap="round" style={{ transition: "stroke-dashoffset 1s linear" }} />
          </svg>
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Space Mono',monospace", fontSize: 26, color: "#fff", fontWeight: 700 }}>
            {mm}:{ss}
          </div>
        </div>
...
function HabitChart({ habitData, days, allHabits }) {
  const values = days.map((day) => {
    ...
    const done = allHabits.filter((h) => d[h]).length;
    ...
    <div style={{ background: "#0d0d0d", border: "1px solid #1f1f1f", borderRadius: 10, padding: 12 }}>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
        ...
        {[0, 25, 50, 75, 100].map((g) => {
          const y = PAD.top + iH - (g / 100) * iH;
          return (
            <g key={g}>
              <line x1={PAD.left} y1={y} x2={W - PAD.right} y2={y} stroke="#1f1f1f" strokeWidth="1" />
              <text x={PAD.left - 4} y={y + 4} textAnchor="end" fontSize="9" fill="#4b5563" fontFamily="Space Mono,monospace">{g}%</text>
            </g>
          );
        })}
...
export default function SecondBrain() {
  ...
  useEffect(() => {
    api.load()
      .then((doc) => {
        setMorningHabits(doc.morningHabits || INITIAL_MORNING);
        setEveningHabits(doc.eveningHabits || INITIAL_EVENING);
        ...
        // Merge saved habitData with any missing days/habits
        const saved = doc.habitData || {};
        const allH  = [...(doc.morningHabits || INITIAL_MORNING), ...(doc.eveningHabits || INITIAL_EVENING)];
        const merged = { ...saved };
        days.forEach((day) => {
          if (!merged[day]) merged[day] = {};
          allH.forEach((h) => { if (merged[day][h] === undefined) merged[day][h] = false; });
        });
        setHabitData(merged);
      })
      .catch(() => {
        // Fallback to defaults if API is unreachable
        setHabitData(buildInitialHabitData(INITIAL_MORNING, INITIAL_EVENING));
      })
  ...
  // ── State setters that also persist ────────────────────────────────────────
  const setAndSaveMorningHabits = (val) => {
    const next = typeof val === "function" ? val(morningHabits) : val;
    setMorningHabits(next);
    debouncedSave({ morningHabits: next });
  };
  const setAndSaveEveningHabits = (val) => {
    const next = typeof val === "function" ? val(eveningHabits) : val;
    setEveningHabits(next);
    debouncedSave({ eveningHabits: next });
  };
  const setAndSaveTodos = (val) => {
    const next = typeof val === "function" ? val(todos) : val;
    setTodos(next);
    debouncedSave({ todos: next });
  };
  const setAndSavePriorities = (val) => {
    const next = typeof val === "function" ? val(priorities) : val;
    setPriorities(next);
    debouncedSave({ priorities: next });
  };
  const setAndSaveHabitData = (val) => {
    const next = typeof val === "function" ? val(habitData) : val;
    setHabitData(next);
    debouncedSave({ habitData: next });
  };

  // ── Habit actions ───────────────────────────────────────────────────────────
  const toggleHabit = useCallback((day, habit) => {
    setAndSaveHabitData((prev) => ({
      ...prev,
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        @media (max-width: 700px) {
          ...
          .nav-scroll { gap: 4px !important; }
        }
        @media (max-width: 900px) and (min-width: 701px) {
          .row1-grid { grid-template-columns: repeat(2, 1fr) !important; }
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div className="row1-grid">
              <Card>
                ...
                <ProgressBar label="Day"     value={dayPct} />
                <ProgressBar label="Quarter" value={Math.round((monthPct + weekPct) / 2)} />
              </Card>
              ...
              <Card>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 20, fontWeight: 600, fontFamily: "'Space Mono',monospace" }}>— °C</div>
                    <div style={{ fontSize: 11, color: "#6b7280" }}>Enable location</div>
                  </div>
                </div>
              </Card>
            </div>
              ...
              <Card>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
                  <input value={newTodo} onChange={(e) => setNewTodo(e.target.value)}
                    onKeyDown={(e) => { if (e.key === "Enter" && newTodo.trim()) { setAndSaveTodos((p) => [...p, { id: Date.now(), text: newTodo.trim(), tag: newTodoTag, done: false }]); setNewTodo(""); } }}
                    placeholder="Add new task..." style={{ ...iStyle, minWidth: 0, fontSize: 12 }} />
              ...
              <Card>
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                  <input value={newPriority} onChange={(e) => setNewPriority(e.target.value)}
                    onKeyDown={(e) => { if (e.key === "Enter" && newPriority.trim()) { setAndSavePriorities((prev) => [...prev, newPriority.trim()]); setNewPriority(""); } }}
                    placeholder="Add priority..." style={iStyle} />
            ...
            <div className="grid-2">
              <Card>
                <ST t="Habits — Today" />
                {[
                  { label: "Morning", habits: morningHabits, group: "morning", newVal: newMorningHabit, setNew: setNewMorningHabit },
                  { label: "Evening", habits: eveningHabits, group: "evening", newVal: newEveningHabit, setNew: setNewEveningHabit },
                ].map(({ label, habits, group, newVal, setNew }) => (
                  <div key={group} style={{ marginBottom: 12 }}>
                    <div style={{ fontSize: 10, color: "#4b5563", fontFamily: "'Space Mono',monospace", letterSpacing: 1, marginBottom: 6 }}>{label.toUpperCase()}</div>
                    {habits.map((habit) => {
                      const done = habitData[today]?.[habit];
                      return (
                        <div key={habit} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid #1a1a1a", flexWrap: "wrap" }}>
                          <input type="checkbox" checked={!!done} onChange={() => toggleHabit(today, habit)} style={{ accentColor: "#22c55e", cursor: "pointer", width: 14, height: 14 }} />
                          <span style={{ fontSize: 12, flex: 1, color: done ? "#86efac" : "#d1d5db", fontWeight: done ? 600 : 400, minWidth: 80 }}>{habit}</span>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                      <input value={newVal} onChange={(e) => setNew(e.target.value)}
                        onKeyDown={(e) => { if (e.key === "Enter") { addHabit(group, newVal); setNew(""); } }}
                        placeholder={`Add ${label.toLowerCase()} habit...`} style={{ ...iStyle, fontSize: 11 }} />
            ...
            {[
              ...
              <Card key={group}>
                ...
                {habits.map((habit) => {
                  const done = habitData[today]?.[habit];
                  return (
                    <div key={habit} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid #1a1a1a", flexWrap: "wrap" }}>
                      <input type="checkbox" checked={!!done} onChange={() => toggleHabit(today, habit)} style={{ accentColor: "#22c55e", cursor: "pointer", width: 16, height: 16 }} />
                      <span style={{ fontSize: 13, width: 120, color: done ? "#86efac" : "#d1d5db", fontWeight: done ? 600 : 400 }}>{habit}</span>
                      <HabitDotGrid habitData={habitData} habit={habit} days={days} onToggle={toggleHabit} />
                      <DelBtn onClick={() => deleteHabit(group, habit)} />
                    </div>
                  );
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 12 }}>
                  <input value={newVal} onChange={(e) => setNew(e.target.value)}
                    onKeyDown={(e) => { if (e.key === "Enter") { addHabit(group, newVal); setNew(""); } }}
                    placeholder={`Add ${label.toLowerCase()} habit...`} style={iStyle} />
                  <AddBtn onClick={() => { addHabit(group, newVal); setNew(""); }} />
                </div>
              </Card>
            ))}
        ...
        {tab === "todos" && (
          <Card>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
              <input value={newTodo} onChange={(e) => setNewTodo(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter" && newTodo.trim()) { setAndSaveTodos((p) => [...p, { id: Date.now(), text: newTodo.trim(), tag: newTodoTag, done: false }]); setNewTodo(""); } }}
                placeholder="Add new task... (press Enter)" style={{ ...iStyle, minWidth: 180 }} />
              <TagSelector current={newTodoTag} onChange={setNewTodoTag} />
              <AddBtn onClick={() => { if (newTodo.trim()) { setAndSaveTodos((p) => [...p, { id: Date.now(), text: newTodo.trim(), tag: newTodoTag, done: false }]); setNewTodo(""); } }} />
            </div>
            {todos.map((t) => (
